...
def _mutated_unresolvable_artifact(tmp_path: Path) -> Capability:
    """Load the real artifact, mutate step 0's target's ROLE in memory to something no element on
    any page can have, write it to tmp_path, and read it back -- never hand-write an artifact
    file.

    Every one of the six resolution strategies filters its candidates by role first, RELATIONAL
    included (surface/locator.py's _STRATEGY_FUNCS), so a bad role defeats all six regardless of
    whatever else the descriptor carries. A name+ordinal-only mutation (this helper's original
    form) stopped being enough once a later re-recording of this same goal carried a relational
    hint the mutation left untouched: RELATIONAL still found the labelled row and resolved anyway
    (measured: this test passed against a hand-built Observation with no row/cell structure to
    exercise RELATIONAL against, then failed live against the real fixture, which has exactly
    that structure -- see docs/adr/0009's addendum).
    """
    original = Capability.model_validate_json(ARTIFACT_PATH.read_text(encoding="utf-8"))
    mutated_target = original.steps[0].target.model_copy(update={"role": "not-a-real-role"})
...
# real artifact still replays successfully despite an (in-memory, constructed) perception_version
# mismatch.
# ------------------------------------------------------------------------------------------
...
def test_t9_live_real_artifact_replays_successfully_despite_perception_version_mismatch(
    ...
    """A real artifact whose recorded perception_version differs from the one running now must
    still replay successfully end to end. `artifacts/` is regenerated by every real discover run
    (a second successful run of the same goal writes a new version rather than overwriting the
    old one, but its perception_version is whatever the run that produced it recorded), so this
    test builds the mismatch itself -- derived from PERCEPTION_VERSION, never hardcoded to a
    specific number -- instead of depending on the artifact currently on disk happening to be
    stale (docs/adr/0009's addendum).
    """
    if not _fixture_app_reachable():
        pytest.skip(
            "the legacy_bank fixture app is not reachable on 127.0.0.1:5055; start it with "
    ...
    mismatched = capability.model_copy(
        update={
            "provenance": capability.provenance.model_copy(
                update={"perception_version": PERCEPTION_VERSION - 1}
            )
        }
    )
    mismatched_path = tmp_path / "mismatched-perception-version.json"
    mismatched_path.write_text(mismatched.model_dump_json(indent=2), encoding="utf-8")

    result = replay_engine.replay(mismatched_path, {}, POLICY_PATH, evidence_base_dir=tmp_path)
...
def test_t8_locator_failure_classifies_stale_perception_vs_locator_unresolved() -> None:
    """Both mismatch and match are built from PERCEPTION_VERSION, never hardcoded to a specific
    number: `artifacts/` is regenerated by every real discover run, so the file on disk right
    now is whatever perception_version that run happened to record, not a frozen historical
    fixture (docs/adr/0009's addendum)."""
    capability = Capability.model_validate_json(ARTIFACT_PATH.read_text(encoding="utf-8"))

    # Versions differ: STALE_PERCEPTION.
    stale = capability.model_copy(
        update={
            "provenance": capability.provenance.model_copy(
                update={"perception_version": PERCEPTION_VERSION - 1}
            )
    ...
    assert _classify_locator_failure(stale) == FailureCategory.STALE_PERCEPTION
