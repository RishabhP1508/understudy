...
# --------------------------------------------------------------------------------------
# 11. THE DRIFT CASE (fact D), using the REAL recorded descriptor values from
# artifacts/look-up-member-12345-and-read-their-current-savings-balance.v1.json.
# --------------------------------------------------------------------------------------


def _real_step0_target() -> TargetDescriptor:
    artifact = json.loads(ARTIFACT_PATH.read_text(encoding="utf-8"))
    return TargetDescriptor(**artifact["steps"][0]["target"])


def test_drift_the_recorded_empty_named_login_field_still_resolves_by_ordinal() -> None:
    """Step 0 of the real recorded Phase 2 artifact targets role=textbox, name="", ordinal=0 --
    the login username field, as Phase 2 perceived it before Phase 3 added name derivation.
    Phase 3 now perceives that same, byte-identical field as name="Username", which alone broke
    the recorded step (verified live: five identical hard_failure results on step 0). This test
    has no browser and no live server, so the login page is stood in with a small CONSTRUCTED
    Observation shaped the way Phase 3 perceives it today: no textbox is named "" any more.
    """
    recorded = _real_step0_target()
...
# --------------------------------------------------------------------------------------
# 12. THE ROUND-2 REGRESSION: the REAL recorded step 3 descriptor (ordinal=null, not ordinal=0
# like step 0/1 above) resolves via ROLE_ORDINAL when its role-filtered pool is a singleton, even
# though no ordinal was ever recorded. A live end-to-end replay against the fixture server proved
# this case is real, not hypothetical: step 3's recorded name="" (Phase 2) now perceives as
# name="Member ID" (Phase 3 derivation), and the Member ID field is the only textbox on that page,
# so every name-based rung reports zero candidates and ROLE_ORDINAL is the first rung to see it.
# --------------------------------------------------------------------------------------


def test_step3_real_descriptor_no_ordinal_resolves_via_role_ordinal_singleton() -> None:
    """CONSTRUCTED: the real recorded step 3 target is role=textbox, name="", ordinal=null --
    the Member ID search field, as Phase 2 perceived it. Phase 3 now derives name="Member ID" for
    that same, unchanged field (fact D's exact mechanism, one step later in the same flow), so
    ROLE_NAME_EXACT and ROLE_NAME_NORMALIZED both see zero candidates. This observation is built
    by hand with exactly one textbox (the drifted-name field) so the case is isolated: a
    role-filtered pool of one is a unique match even with no ordinal recorded, because there is
    no second candidate to bet on positionally.
    """
    artifact = json.loads(ARTIFACT_PATH.read_text(encoding="utf-8"))
    recorded = TargetDescriptor(**artifact["steps"][3]["target"])
    assert recorded.role == "textbox"
    assert recorded.name == ""
    assert recorded.ordinal is None
